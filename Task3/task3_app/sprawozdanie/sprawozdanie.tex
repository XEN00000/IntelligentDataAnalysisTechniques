...
Jak wykazano w tabeli \ref{tab:best_results}, architektura ResNet50 osiągnęła wynik $\text{ROC AUC}_{\text{Macro}} = 0.9943$, co wskazuje na niemal perfekcyjną zdolność do separacji klas w optymalnych warunkach treningowych.

\begin{figure}[H]
    \centering
    \begin{subfigure}{0.48\textwidth}
        \includegraphics[width=\linewidth]{img/roc_resnet50_train85_test15.png} % Z pliku
        \caption{ResNet50 (Wariant train85\_test15)}
        \label{fig:roc_resnet}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.48\textwidth}
        \includegraphics[width=\linewidth]{img/roc_placeholder.png} % PODMIEŃ ŚCIEŻKĘ
        \caption{Porównanie krzywych dla pozostałych modeli}
        \label{fig:roc_other}
    \end{subfigure}
    \caption{Krzywe ROC oraz wartości AUC (Micro/Macro) dla wybranych konfiguracji badanych modeli.}
...
Dla zobrazowania praktycznego działania wyuczonych sieci, na rysunku \ref{fig:predictions_grid} przedstawiono przykładowe predykcje obrazów ze zbioru testowego z nałożonymi etykietami rzeczywistymi (T: \textit{True}) oraz przewidywanymi przez model (P: \textit{Predicted}). Kolorem zielonym oznaczono poprawne klasyfikacje, natomiast kolorem czerwonym przypadki błędne.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{img/predictions_mobilenetv2_train95_test5.png} % Z pliku
    \caption{Przykładowa siatka predykcji obrazów dla modelu MobileNetV2 (konfiguracja train95\_test5). Poprawne przypisania zaznaczono na zielono, błędy na czerwono.}
    \label{fig:predictions_grid}
\end{figure}

Dla optymalnej konfiguracji modelu \textbf{ResNet50}, przykładowe zdjęcia zostały zaklasyfikowane w następujący sposób:

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{img/predictions_resnet50_best_placeholder.png} % PODMIEŃ ŚCIEŻKĘ
    \caption{Zestawienie predykcji obrazów dla najlepszej konfiguracji sieci ResNet50.}
